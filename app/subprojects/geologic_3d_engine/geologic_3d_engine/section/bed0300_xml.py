"""Strict intake adapter for MLIT BED0300 (DTD 3.00) borehole XML."""
...
SUPPORTED_DTD_VERSION = "3.00"
LITHOLOGY_ROW = "岩石土区分"
BOTTOM_TAG = f"{LITHOLOGY_ROW}_下端深度"
LABEL_TAG = f"{LITHOLOGY_ROW}_岩石土名"


def parse_bed0300_xml(path, *, horizontal_crs, vertical_datum,
    ...
    """Parse one BED0300 record without silently changing datum or terms."""
    source = Path(path)
    raw = source.read_bytes()
    if not raw or len(raw) > MAXIMUM_XML_BYTES:
        raise ValueError("BED0300 XML size is invalid")
    text = _decode_xml(raw)
    if re.search(r"<!\s*(?:ENTITY|ATTLIST)\b", text, re.IGNORECASE):
        raise ValueError("BED0300 XML entities and attribute declarations are prohibited")
    text = re.sub(r"^\ufeff?<\?xml[^>]*\?>", "", text, count=1, flags=re.IGNORECASE)
    text = re.sub(r"<!DOCTYPE[^>]*>", "", text, count=1, flags=re.IGNORECASE)
    try:
        root = ET.fromstring(text)
    except ET.ParseError as exc:
        raise ValueError("BED0300 XML is not well formed") from exc
    if root.tag != "ボーリング情報" or root.attrib.get("DTD_version") != SUPPORTED_DTD_VERSION:
        raise ValueError("only BED0300 DTD version 3.00 is supported")
    ...
    core = root.find("コア情報")
    if core is None:
        raise ValueError("BED0300 core information is missing")
    intervals = []
    top = 0.0
    ...
    result = normalize_borehole({
        ...
        "exchangeFormatVersion": "BED0300-DTD-3.00",
    ...
    result.update({
        ...
        "parserBoundary": "BED0300StructureOnly_NoAutomaticDatumConversionOrTermNormalization",
    })
    return result
